...
function animate(element, target) {
    var step = 10;
    var time = 10;
    var gap = (Math.abs(target - element.offsetLeft) / slider_item_width); // 由第1张跳第3张的跨序 gap2
    
    
    if (gap > 1) { // 跨序时，时间缩短、速度加快
        step = step * gap;
        time = time / gap;
    }
    if (element) {
        step = (element.offsetLeft > target) ? -step : step; // 判断方向(左、右)
        
        clearInterval(intervalId); // 把上次(上一页)的定时器清除
        setCurrentActiveIndicator(curIndex); // 设置下方激活点
        intervalId = setInterval(function () {// 没time移动step距离
            if ((step < 0) && (Math.abs(element.offsetLeft + step) < Math.abs(target))) { // 下一页 并且移动距离小于需要移动距离
                element.style.left = element.offsetLeft + step + "px";
            } else {
                if (Math.abs(target - element.offsetLeft) > Math.abs(step)) { // 上一页
                    element.style.left = element.offsetLeft + step + "px";
                } else { // 一页移动结束
